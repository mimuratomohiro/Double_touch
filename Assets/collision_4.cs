...
public class collision_4 : MonoBehaviour
{
	...
	private void OnCollisionStay(Collision collision)
	{
		...
		angle_now = -90 + this.transform.eulerAngles.x - angle_root;
		var y = 4.0f*Math.Sin(Math.PI * angle_root/180);
		var z = 4.0f*Math.Cos(Math.PI * (180-angle_root)/180);
		var ang =new Vector3(0.0f, (float)y, (float)z); 
		var pos = Vector3.Magnitude(contact.point - Vector3.forward*2 - ang);
		if (angle_root < 0) {
			angle_root += 360;
		}
		if (angle_now < 0) {
