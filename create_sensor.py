...
if __name__ == '__main__':
	...
	point_list1 = []
	point_list2 = []
	distance1   = np.ones((points.size,points.size))
	distance2   = np.ones((points.size,points.size))

	for point1, point2 in zip(Cube_2_point,Cube_3_point):
		double_touch_point1 = np.zeros(points.size)/5
		double_touch_point2 = np.zeros(points.size)/5

		for i in range(points.size):
			if(point1 + hand_size > points[i] and points[i] > point1 - hand_size ):
				double_touch_point1[i] = 1

			if(point2 + hand_size > points[i] and points[i] > point2 - hand_size ):
